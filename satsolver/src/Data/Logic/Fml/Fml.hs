...
translateAlgo fml@(And (And r s ) (q)) = if isOneClause s == True then  switchAlgo (translation r s q)  else translateAlgo( translation r s q )  -- déplace
...
switchAlgo fml@(And p q) =if isOneClause p == False && isOneClause q == True then switcher fml else if  isOneClause p == False && isOneClause q == False then translateAlgo fml else switchAlgo q
toCCNF :: Fml a -> Fml a
